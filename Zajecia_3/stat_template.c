...
void mean_variance(const double v[], double *mean, double *variance, int n)
{
    ...
    *mean/=n;
    for(int i=0; i<n; i++)  *variance+=(*mean-v[i])*(*mean-v[i]);
    *variance/=n;
}
...
void bernoulli_gen(int v[], int bernoulli_percentage, int n)
{
    ...
    for(int i=0; i<n; i++){
        number=rand();
        if((number%100)<bernoulli_percentage){
            v[i]=1;
        }
        else{
            v[i]=0;
...
void calculate_cdf(double cdf[], int trials)
{
    ...
    for(int i=1; i<TWO_DICE_SUM; i++)  cdf[i]+=cdf[i-1];
...
void monty_hall(int *wins, double *win_probability, int n) {
	*wins = 0;
    for(int i = 0; i < n; i++) {
		...
		int remaining = choice; // your second choice (if you switch)
